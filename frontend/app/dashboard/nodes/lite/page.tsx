...
import { ArrowLeft, Cloud, KeyRound, Copy, CheckCircle2, AlertCircle, ArrowRight } from 'lucide-react'
import api from '@/lib/api'

export default function LiteNodeSetupPage() {
  ...
  const handleGenerateKey = async () => {
    ...
    setCreatingKey(true)
    try {
      const created = await api.apiKeys.create(`lite-node-${Date.now()}`)
  ...
  const handleCreateNode = async () => {
    ...
    try {
      const node = await api.nodes.createLite()
      setCreatedNode(node)
    } catch (e: any) {
      setError(e?.message || 'Failed to create Lite node')
    } finally {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <Cloud className="w-9 h-9 text-cyan-500" />
            <h1 className="text-4xl font-bold">Lite Node</h1>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-effect rounded-lg p-6 border border-cyan-500/20">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="text-xl font-semibold">You selected Lite Node</div>
          ...
          <div className="glass-effect rounded-lg p-6 border border-cyan-500/20">
            ...
            <p className="text-sm text-gray-400 mb-4">
              After you’ve entered the key in the extension/app, create your Lite node record here and open its dashboard to view performance.
            </p>
